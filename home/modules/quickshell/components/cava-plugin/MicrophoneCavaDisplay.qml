// Microphone audio visualizer component
import QtQuick
...
Row {
    ...
    property color barGradientColor: "#ff6b6b"
    ...
    MicrophoneCavaProvider {
        ...
        Component.onCompleted: {
            console.log("MicrophoneCavaDisplay: MicrophoneCavaProvider created with bars:", bars, "barCount:", root.barCount);
        }
        
        onBarsChanged: {
            console.log("MicrophoneCavaDisplay: bars changed to:", bars);
        }
        ...
        onValuesChanged: {
            ...
            for (let i = 0; i < repeater.count; i++) {
                if (repeater.itemAt(i)) {
                    repeater.itemAt(i).normalizedValue = values[i]; // Store normalized value directly
                }
            }
        }
    }
