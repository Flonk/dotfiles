...
- Obsidian vault: `/home/flo/repos/personal/dotfiles/obsidian` (the vault root — there is NO nested `Vault/` folder; never create one, the vault syncs via Obsidian Sync and a stray folder duplicates the whole vault remotely) — CLI: `obsidian`. **Always use a subcommand** (e.g. `obsidian help`, not bare `obsidian`). **Do NOT touch `personal/z/`.** **There is no `obsidian edit` command** — to edit vault files, use the `Read` + `Edit` tools directly on the file path (e.g. `/home/flo/repos/personal/dotfiles/obsidian/claude/projects.md`).
