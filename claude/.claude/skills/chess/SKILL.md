...
It resolves a name to a position (shortest matching ECO line), then renders the board with a **lime mainline arrow** (100%) + up to six faint **blue arrows** (flat 15% opacity) for the next book moves. "Popularity" = how many named ECO lines run through each move (offline proxy; no engine, no network). Output JSON: `{name, fen, turn, continuations[], image}` where each continuation is `{san, uci, name, count, pct}`, most popular first.
